...
simpleSimulator <- function(averageInterarrivalTime = 2.5,
  ...
  summaryDF <- progressDF %>% 
    summarise(.by = replication, 
                         ...
                         numberCompleted = sum(!is.na(throughPutTime)),
                          verifQueue = mean(queueVerif, na.rm = TRUE), 
                          labelQueue = mean(queueLabel, na.rm = TRUE), 
                          dispQueue = mean(queueDisp, na.rm = TRUE),
                          finCheckQueue = mean(queueFinCheck, na.rm = TRUE),
    ...
    dplyr::select(replication, numberItems, numberCompleted, 
                  percentageCompleted, verifQueue, labelQueue, dispQueue, 
                  finCheckQueue, meanQueue, meanThroughput) %>% 
    as.data.frame()
  ...
  resultsVector <- c(averageInterarrivalTime = averageInterarrivalTime,
                     ...
                     meanVerifQueue = weighted.mean(summaryDF$verifQueue, summaryDF$numberItems),
                     meanLabelQueue = weighted.mean(summaryDF$labelQueue, summaryDF$numberItems),
                     meanDispQueue = weighted.mean(summaryDF$dispQueue, summaryDF$numberItems),
                     meanFinCheckQueue = weighted.mean(summaryDF$finCheckQueue, summaryDF$numberItems),
