...
e2Df <- lapply(0:9, function(x) simpleSimulator(numberOfRepetitions = 1000,
...
###################
#Drawing of curves:
plot(e2Df$durationFinCheck, e2Df$meanThroughput, col = 'grey', 
...
locMinTS <- uniroot(throughputFunctS1, interval = c(10, 13))$root[1]
abline(v = locMinTS, col = 'grey', lwd = 1, lty = 'dashed')
abline(h = throughputFunctS(locMinTS), col = 'grey', lwd = 1, lty = 'dashed')
#Waiting times:
lines(e2Df$durationFinCheck, e2Df$meanQueue, col = 'grey', lty = 'dashed')
...
locMinWS <- uniroot(waitingFunctS1, interval = c(10, 13))$root[1]
...
resultDf <- data.frame(metric = c('minimum throughput time (red)',
                                  ...
                                  'duration of steps 1, 2, and 4 at minimum (red)'),
